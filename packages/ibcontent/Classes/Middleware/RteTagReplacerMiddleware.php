...
namespace Rms\Ibcontent\Middleware;
    ...
    private function renderTagTemplate(array $options): string
    {
        ...
        switch ($options[0]) {
            case 'TAG':
                ...
                $this->template->assignMultiple(values: array('genericTag' => $options));

                return $this->template->render();
            case 'YT':
                $this->standaloneView->setTemplate('YoutubeTag');
                $this->template = $this->standaloneView;
                $this->template->assignMultiple(array('youtubecode' => $options[1], 'startAt' => $options[2] ?? 0, 'videoTitle' => $options[3] ?? ''));
